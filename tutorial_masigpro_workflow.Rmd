...
# r-squared threshold: proportion of variance explained by the regression model
# 0.7 is the authors' recommended default (good balance of stringency and yield)
# alternative values: 0.6 (more genes, less confident fits), 0.8 (fewer genes, higher confidence)
# adjust based on your dataset size and downstream analysis requirements
rsq_threshold <- 0.7
...
# extract gene names from the significant profiles matrix
# sigs$sig.genes contains results organized by comparison group (here: T21vsD21)
# sig.profiles holds the expression profiles for genes passing the rsq threshold
sig_genes <- rownames(sigs$sig.genes$T21vsD21$sig.profiles)

# output: number of genes showing significant genotype-specific temporal patterns
# these genes have expression trajectories that differ between conditions over time
...
# number of clusters for gene grouping
# start with k=10 and adjust based on cluster sizes and biological interpretability
# too many clusters may result in groups with very few genes; too few may obscure patterns
k_clusters <- 10

# remove non-numeric columns from experimental design for clustering
...
clusters <- maSigPro::see.genes(
  ...
  k = k_clusters,
  # distance metric: "correlation" groups genes with similar expression patterns
  # alternative: "euclidean" considers absolute expression differences
  dis = "correlation",
  # clustering algorithm: "hclust" (hierarchical) or "kmeans"
